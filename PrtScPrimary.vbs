		Option Explicit
		Dim FObj,WshShell,WordObj,ImageFolderPath,LocalDocumentPath,BaseSheredPath,DateSpan,StaffCode
...
Call LocalStorageScheduleClear(LocalDocumentPath)
Call DeleteUnwantedScreenshot(ImageFolderPath)		
Call Action()
...
'Function to clear Local Storage at a Certain Day Interval (DateSpan)
Function LocalStorageScheduleClear(ByVal Path)
	Dim objLocalFolder,Folders,objFolders,fromDate,toDate
	Set objLocalFolder = FObj.GetFolder(Path)
	Set Folders = objLocalFolder.SubFolders
	For Each objFolders in Folders
		If IsDate(objFolders.Name) Then 
			fromDate= CDate (objFolders.Name)
			toDate=Date()
			If  DateDiff("d",fromDate,toDate)=>DateSpan Then 
				objFolders.Delete True	
			End If 
		End If 
	Next
End Function

'Function to delete the last unwanted screenshot captured while taking action
Function DeleteUnwantedScreenshot(ByVal Path)
		Dim RecentFile,file,objFolder
